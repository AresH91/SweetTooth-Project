...
const SearchResult = () => {
    ...
    useEffect(() => {
        axios.get(`http://localhost:8000/api/recipes/find/${category}`)
            .then(res => {
                setRecipes(res.data)
                setLoaded(true)
            })
        ...
        <div className='container-xxl px-md-5 bg-white shadow-lg'>
            ...
            <nav aria-label='breadcrumb'>
                <ol className='breadcrumb'>
                    ...
                    <li className='breadcrumb-item-active' aria-current="page">&nbsp; /&nbsp; <Link to='/categories'>Categories <i className='bi bi-tag'></i></Link></li>
                    <li className='breadcrumb-item-active' aria-current="page">&nbsp; /&nbsp; Results</li>
                </ol>
            </nav>
            {loaded && <ListedRecipes recipes={recipes} />}
            <div><br></br></div>
        </div>
    )
}
